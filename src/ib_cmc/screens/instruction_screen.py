...
PRE_INSTRUCTION_TEXT = (
    # 수정요청(260821) 1번: 요청서가 표시한 두 지점(마음에 / 얼마나,
    # 느끼는 점을 / 바탕으로)에서 반드시 끊고, 한 단어만 남는 줄이 생기지
    # 않도록 앞쪽 줄바꿈도 함께 고정했다.
    "본 과제에서 여러분은 일상에서 흔히 겪을 수 있는<br>"
    "친구와의 문자 메세지 대화 상황들을 살펴보게 될 겁니다.<br>"
    "<u>화면에 제시되는 메세지 내용을 읽고 제시되는 해석들이 여러분의 마음에<br>"
    "얼마나 떠오르는지 점수를 매겨주시면 됩니다.</u>\n\n"
    "이 과제에는 정답이 없습니다. 평소 여러분이 친구와 문자를 주고받을 때 느끼는 점을<br>"
...
# 수정요청(260821) 6번: 사후 측정 재설명 슬라이드 — 줄바꿈, 밑줄, 내용 수정.
POST_INSTRUCTION_TEXT = (
    # 요청서의 줄바꿈 위치를 따르되, 화면 폭(1000px)에 맞게 둘째 줄만 조금 앞에서 끊었다.
    "지금부터는 방금과 동일하게 일상에서<br>"
    "흔히 겪을 수 있는 친구와의 문자 메시지 대화 상황을 본 후 메시지 내용과 이어지는<br>"
    "해석들이 여러분의 마음속에 얼마나 떠오르는지 점수를 매겨주시면 됩니다.\n\n"
    "아까 체크했던 점수를 생각하기보다, 지금 문장을 읽으면서<br>"
    "<u>실제로 내 마음속에 떠오르는 '현재의 느낌'</u> 그대로 편안하게 점수를 매겨주세요.\n\n"
    "아까 체크했던 점수와 동일하여도 되고 달라져도 괜찮습니다.\n\n"
    "이번 역시 정답은 없으며 솔직하게 깊게 고민하지 말고 여러분의 마음속에<br>"
    "<u>'즉각적으로' 떠오르는 정도</u>를 선택해주세요.\n\n"
